...
`AGENTS.md` hard rule #11 is the full spec. The one file-at-a-time
anti-pattern below — a `fizruk` page reaching for `ring-routine` — is
...
- `AGENTS.md` hard rule #10 — no arbitrary hex colors in `className`.
- `AGENTS.md` hard rule #11 — module-accent containment.
