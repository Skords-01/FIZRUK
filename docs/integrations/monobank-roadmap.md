...
| **D** | Push-нотифікації                     | плюс плюс плюс        | висока     | середній | 🟡 partial — інфра є: `apps/server/src/lib/webpushSend.ts` (timeout / retry / circuit breaker), міграція `006_push_devices.sql`. Залишається wiring `onTransactionPersisted() → sendToUser()` для Mono-events. |
...
**Рекомендована черговість:** A → B → C → D (довести до кінця) → E.
...
- `useFlag("mono_webhook")` всі виклики (зараз: `FinykApp.tsx:45`, `useMonobank.ts:161`)
