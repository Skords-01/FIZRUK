...
| #     | PR                                   | Ефект для користувача | Складність | Ризик    |
| ----- | ------------------------------------ | --------------------- | ---------- | -------- |
| **A** | Cleanup legacy polling               | (невидимий)           | низька     | низький  |
| **B** | Backfill UX (Діагностика + progress) | плюс                  | низька     | низький  |
| **C** | Auto-categorization по MCC           | плюс плюс             | середня    | середній |
| **D** | Push-нотифікації                     | плюс плюс плюс        | висока     | середній |
| **E** | Webhook secret rotation              | (захист)              | низька     | низький  |
...
- `useFlag("mono_webhook")` всі виклики (зараз: `FinykApp.tsx:219`, `useMonobank.ts:158`)

**Server (`apps/server/src/`):**

- `modules/mono.ts` — legacy proxy `/api/mono?path=/personal/...` (не потрібен, server тепер сам ходить у Monobank через `connection.ts`)
