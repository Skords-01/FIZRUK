...
function formatDueDate(dueDate) {
  ...
  const days = Math.ceil((date - new Date(now.getFullYear(), now.getMonth(), now.getDate())) / 86400000);
...
export function DebtCard({ name, emoji, remaining, paid, total, onDelete, onLink, linkedCount, isReceivable, dueDate }) {
  ...
  return (
    <div className="bg-panel border border-line rounded-xl p-4 mb-3">
      <div className="flex items-start justify-between mb-3">
        <span className="text-sm font-semibold leading-snug">{emoji} {name}</span>
        <div className="flex items-center gap-2 shrink-0 ml-2">
          <span className={cn("text-sm font-bold tabular-nums", isReceivable ? "text-success" : "text-danger")}>
      ...
      <div className="h-1.5 bg-line rounded-full overflow-hidden">
        <div
          className={cn("h-full rounded-full transition-all duration-500", isReceivable ? "bg-success" : "bg-primary")}
          style={{ width: `${pct}%` }}
        />
      </div>
      <div className="text-xs text-subtle mt-2">
        {isReceivable ? "Отримано" : "Сплачено"} {paid.toLocaleString("uk-UA", { maximumFractionDigits: 0 })} з {total.toLocaleString("uk-UA")} ₴
      </div>
      {dueText && (
        <div className={cn("text-xs mt-1", isOverdue ? "text-danger" : "text-muted")}>
