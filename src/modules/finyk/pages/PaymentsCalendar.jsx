...
export function PaymentsCalendar({ mono, storage }) {
  ...
  // Pad days to start on correct weekday (Mon=0)
  const firstDow = (monthStart.getDay() + 6) % 7; // convert Sun=0 to Mon=0
    ...
    <div className="flex-1 overflow-y-auto">
      <div className="max-w-2xl mx-auto px-4 pt-4 pb-16 space-y-3">
        ...
        <div className="bg-panel border border-line rounded-xl p-4">
          <div className="text-xs text-subtle mb-2">Календар платежів · {now.toLocaleDateString("uk-UA", { month: "long", year: "numeric" })}</div>
              ...
              return (
                <div
                  ...
                  className={cn(
                    ...
                    isToday ? "border-primary bg-primary/10" : list.length > 0 ? "border-line bg-[#0f172a]" : "border-transparent"
                  )}
                >
                  <div className={cn("text-xs font-semibold", isToday ? "text-[#a5b4fc]" : "text-muted")}>{day}</div>
                  {list.length > 0 && (
