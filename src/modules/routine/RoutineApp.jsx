...
export default function RoutineApp({ onBackToHub, onOpenModule } = {}) {
  ...
  const [habitDraft, setHabitDraft] = useState({ name: "", emoji: "✓", tagIds: [], categoryId: null });
...
function SettingsSection({
  ...
  return (
    <div className="space-y-4 pb-4">
      ...
      <section className="bg-panel border border-line/60 rounded-2xl p-4 shadow-card space-y-3">
        <h2 className="text-xs font-bold text-subtle uppercase tracking-widest">Нова звичка</h2>
        ...
        <Button
          ...
          onClick={() => {
            const tagIds = habitDraft.tagIds || [];
            setRoutine((s) =>
              createHabit(s, {
                name: habitDraft.name,
                emoji: habitDraft.emoji,
                tagIds,
                categoryId: habitDraft.categoryId,
              }),
            );
            setHabitDraft({ name: "", emoji: "✓", tagIds: [], categoryId: null });
          }}
        >
          Додати звичку
        </Button>
        ...
        <ul className="space-y-2">
          {routine.habits
            ...
            .map((h) => (
              <li
                key={h.id}
                className="text-sm flex justify-between gap-2 border-b border-line/40 pb-2 last:border-0 last:pb-0 font-medium"
              >
                <span>
                  {h.emoji} {h.name}
                </span>
